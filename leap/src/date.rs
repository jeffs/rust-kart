...
const DAYS_PER_WEEK: u8 = week::DAYS.len() as u8;
...
impl Date {
    ...
    pub fn day_of_week(self) -> week::Day {
        // +1 because January 1st of the year 1 CE would have been a Monday.
        week::DAYS[(self.count_days() + 1) % week::DAYS.len()]
    }
...
mod tests {
    ...
    fn day_of_week_works() {
        for (year, day) in [
            ...
            assert_eq!(
                Date::from_ymd(year, 1, 1).map(Date::day_of_week),
                Ok(day),
                "January 1st of year {year}"
            );
        }
        let date = Date::from_ymd(1, 1, 1).expect("January 1st of the year 1 CE");
        assert_eq!(date.day_of_week(), week::Day::Mon);
        let date = Date::from_ymd(2, 1, 1).expect("January 1st of the year 2 CE");
        assert_eq!(date.day_of_week(), week::Day::Tue);
        let date = Date::from_ymd(3, 1, 1).expect("January 1st of the year 3 CE");
        assert_eq!(date.day_of_week(), week::Day::Wed);
        let date = Date::from_ymd(4, 1, 1).expect("January 1st of the year 4 CE");
        assert_eq!(date.day_of_week(), week::Day::Thu);
        let date = Date::from_ymd(5, 1, 1).expect("January 1st of the year 5 CE");
        assert_eq!(date.day_of_week(), week::Day::Sat);
    }
}
